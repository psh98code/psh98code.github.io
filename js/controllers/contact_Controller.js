...
const txtTitle = {"es": {txt: "Contacta conmigo"}, "en": {txt: "Contact me"}}
...
export class ContactController{
    ...
    renderDesktopContent(){
        ...
        document.getElementById("sectionDesktop").innerHTML = html;
         ...
         // ---------------------------------------------- CAMBIOS EN EL SELECT TIPO DE CONTACTO -------------------------------------------------
        ...
        contactTypeSelect.addEventListener("change", (e) => {
         ...
         // ---------------------------------------------- CAMBIOS EN EL SELECT TIPO DE OFERTA -------------------------------------------------
        ...
        document.getElementById("btnSubmit_Desktop").addEventListener("click", (e) => {
         ...
         // ---------------------------------------------- BOTON DE CERRAR DIV DE ERRORES -------------------------------------------------
    ...
    renderMobileContent(){
        ...
        let html = `
            <div id="pages_contact_Mobile_DivMain">
                <div>
                    <h1 class="text-4xl md:text-6xl font-bold text-gray-200 mb-2 text-center">${txtTitle[lang].txt}</h1>
                    <p class="text-base md:text-lg text-gray-400 mb-6 text-center mx-auto">${contactContent[lang].txt}</p>
                </div>
                <div id="cardContactMe">
                    <h2 class="text-2xl md:text-3xl font-semibold text-gray-200 mb-3 text-center">${txtEmail[lang].txt}</h2>
                    <form id="contactForm_Mobile" class=" p-6 shadow-lg rounded-lg" action="https://formspree.io/f/mldpgyny" method="POST">
                        <div class="flex flex-col">
                            <div>
                                <label for="contactType_Mobile" class="font-semibold text-lg">${typeOfContact[lang].txt}</label>
                                <select id="contactType_Mobile" name="Tipo de contacto" class="border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
                                    <option value="none" selected>${optionsOfSelect[lang].txt0}</option>
                                    <option value="empresa">${optionsOfSelect[lang].txt1}</option>
                                    <option value="particular">${optionsOfSelect[lang].txt2}</option>
                                </select>
                            ...
                            <div>
                                <label for="offerType_Mobile" class=" font-semibold text-lg">${offerType[lang].txt}</label>
                                <select id="offerType_Mobile" name="Tipo de oferta" class="border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
                                    <option value="none" selected>${offerType[lang].options.txt0}</option>                                   
                                    <option value="jobOffer">${offerType[lang].options.txt1}</option>
                                    <option value="privateLessons">${offerType[lang].options.txt2}</option>
                                    <option value="privateCourses">${offerType[lang].options.txt3}</option>
                        ...
                        <div id="contactForm_Mobile_Content_DataPersonal">
                        </div>
                        <div id="contactForm_Mobile_Content_DataProyect">
                        </div>
                        <div class="field-group">
                            <input type="submit" value="Enviar" id="btnSubmit_Mobile">
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div id="errorDivMsgContact_Mobile">
                <div id="errorDivMsgContact_Mobile_CARD">
                    <h1 class="text-2xl font-bold text-center text-red-600 mb-4">¡Revisa tus errores!</h1>
                    <div id="errorDivMsgContact_Mobile_Content">
                    </div>
                    <div>
                        <h2 id="errorDivMsgContact_Mobile_CloseBtn">Cerrar</h2>
                    </div>                
                </div>
            </div>
        `;
        ...
        const contactTypeSelect = document.getElementById("contactType_Mobile");
        contactTypeSelect.addEventListener("change", (e) => {
            const contactTexts = {
                "es": {
                    particularName: "Nombre y Apellidos",
            ...
            let html = "";
            document.getElementById("contactForm_Mobile_Content_DataPersonal").innerHTML = "";
            ...
            if(e.target.value == "particular"){
                ...
                `;
                document.getElementById("contactForm_Mobile_Content_DataPersonal").style.display = "flex";
            } 
            else if(e.target.value == "empresa") {
                html = `
                    <div class="field-group flex gap-4">
                ...
                document.getElementById("contactForm_Mobile_Content_DataPersonal").style.display = "none";
            }
            document.getElementById("contactForm_Mobile_Content_DataPersonal").innerHTML = html;
        });

        const proyectTypeSelect = document.getElementById("offerType_Mobile");
        proyectTypeSelect.addEventListener("change", (e) => {
            const offerTexts = {
                "es": {
                    jobTitle: "Puesto laboral",
            ...
            let html = "";
            document.getElementById("contactForm_Mobile_Content_DataProyect").innerHTML = "";
            ...
            switch(e.target.value) {
                ...
                case "none":
                    document.getElementById("contactForm_Mobile_Content_DataProyect").style.display = "none";                                        
                    break;
            }

            if(e.target.value !== "none") {
                document.getElementById("contactForm_Mobile_Content_DataProyect").style.display = "flex";
            }

            document.getElementById("contactForm_Mobile_Content_DataProyect").innerHTML = html;
        });

        // ---------------------------------------------- BOTON DE ENVIAR -------------------------------------------------
        document.getElementById("btnSubmit_Mobile").addEventListener("click", (e) => {
            const lang = document.documentElement.lang || "es"; 
            const contactType = document.getElementById("contactType_Mobile").value;
            const offerType = document.getElementById("offerType_Mobile")?.value;
            const inputs = document.querySelectorAll(".input-field, .text-area-field");
            ...
            let ok = true;
            let errors = [];
            const errorDivContent = document.getElementById("errorDivMsgContact_Mobile_Content");
            ...
            if(!ok){
                e.preventDefault();
                e.stopPropagation();
                document.getElementById("errorDivMsgContact_Mobile").style.display = "flex";
                errorDivContent.innerHTML = "";
                errors.forEach(err => {
                    const p = document.createElement("p");
                    p.textContent = err;
        ...
        document.getElementById("errorDivMsgContact_Mobile_CloseBtn").addEventListener("click", () => {
            document.getElementById("errorDivMsgContact_Mobile").style.display = "none";
        });
    }
}